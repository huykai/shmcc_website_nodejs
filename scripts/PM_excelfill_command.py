...
logging.basicConfig(filename='pm_excel_logger_command.log', level=logging.INFO)
...
if __name__ == '__main__':
    ...
    if ( len(sys.argv) ) > 2 :
        ...
        logging.info("\t params netype: " + str(sys.argv[2])) 
        logging.info("\t params ne: " + str(sys.argv[3]))
        #print("\t run mode : " + str(sys.argv[1])) 
        #print("\t params : " + str(sys.argv[2])) 
        runmode = sys.argv[1]
        formparams['netype'] = sys.argv[2]
        formparams['ne'] = sys.argv[3]
       
    param = PmSqlParam()

    if (not formparams is None):
        try:
            getforminfo(param, json.dumps(formparams))
